...
/**
 * DELETE /api/indicator-templates/{name} against design doc 135 (token, name, hard delete).
 *
 * <br><br>
...
class SystemOverviewDesign135Test {
	...
	class TokenAuthentication {
		...
		void missingTokenReturns401() throws Exception {
			upsert("ToDelete", "{}");
			mockMvc.perform(delete("/api/indicator-templates/ToDelete")).andExpect(status().isUnauthorized());
		}
	}
	...
	class NameValidation {
		...
		void nameLongerThan64Returns422() throws Exception {
			...
			mockMvc.perform(delete("/api/indicator-templates/" + name).header(HttpHeaders.AUTHORIZATION, bearerDemo))
	...
	class DeleteTemplate {
		...
		void unknownNameReturns404() throws Exception {
			mockMvc.perform(delete("/api/indicator-templates/Missing").header(HttpHeaders.AUTHORIZATION, bearerDemo))
		...
		void otherCustomerReturns404AndRowRemains() throws Exception {
			upsert("KeepMine", "{\"k\":1}");
			mockMvc.perform(delete("/api/indicator-templates/KeepMine").header(HttpHeaders.AUTHORIZATION, bearerDemo2))
					.andExpect(status().isNotFound())
					.andExpect(jsonPath("$.errorCode").value(ErrorCodes.NOT_FOUND));

			assertThat(tvIndicatorTemplateRepository.findByCustomerNoAndName(1L, "KeepMine")).isPresent();
			mockMvc.perform(get("/api/indicator-templates/KeepMine").header(HttpHeaders.AUTHORIZATION, bearerDemo))
		...
		void happyPathDeletesAndReturnsSystemDatetime() throws Exception {
			...
			MvcResult result = mockMvc.perform(delete("/api/indicator-templates/Gone")
			...
			mockMvc.perform(get("/api/indicator-templates/Gone").header(HttpHeaders.AUTHORIZATION, bearerDemo))
					.andExpect(status().isNotFound())
					.andExpect(jsonPath("$.errorCode").value(ErrorCodes.NOT_FOUND));

			assertThat(tvIndicatorTemplateRepository.findByCustomerNoAndName(1L, "Gone")).isEmpty();
