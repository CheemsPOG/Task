...
/**
 * GET /api/indicator-templates against design doc 132 (token, customer filter, name-only list).
 *
 * <br><br>
...
class SystemOverviewDesign132Test {
	...
	class TokenAuthentication {
		...
		void missingTokenReturns401() throws Exception {
			mockMvc.perform(get("/api/indicator-templates")).andExpect(status().isUnauthorized());
		}
	}
	...
	class ListRetrieval {
		...
		void emptyListWhenCustomerHasNoTemplates() throws Exception {
			MvcResult result = mockMvc.perform(get("/api/indicator-templates").header(HttpHeaders.AUTHORIZATION, TestAuthSupport.bearerForCustomer(jwtService, "ghost99", 99L)))
		...
		void returnsOnlyCurrentCustomerNamesSortedAsc() throws Exception {
			...
			MvcResult result = mockMvc.perform(get("/api/indicator-templates").header(HttpHeaders.AUTHORIZATION, bearerDemo))
					.andExpect(status().isOk())
					.andReturn();
	...
	private void seed(long customerNo, String name, String content) {
		tvIndicatorTemplateRepository.save(new TvIndicatorTemplate(customerNo, name, content, Instant.now()));
	}
}
