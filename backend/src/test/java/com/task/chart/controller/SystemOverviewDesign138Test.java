...
/**
 * GET /api/chart-templates/{name} against design doc 138 (token, name, DTO).
 *
 * <br><br>
...
class SystemOverviewDesign138Test {
	...
	class TokenAuthentication {
		...
		void missingTokenReturns401() throws Exception {
			mockMvc.perform(get("/api/chart-templates/Dark")).andExpect(status().isUnauthorized());
		}
	}
	...
	class NameValidation {
		...
		void nameLongerThan64Returns422() throws Exception {
			...
			mockMvc.perform(get("/api/chart-templates/" + name).header(HttpHeaders.AUTHORIZATION, bearerDemo))
	...
	class Retrieval {
		...
		void unknownNameReturns404() throws Exception {
			mockMvc.perform(get("/api/chart-templates/Missing").header(HttpHeaders.AUTHORIZATION, bearerDemo))
		...
		void otherCustomerReturns404() throws Exception {
			...
			mockMvc.perform(get("/api/chart-templates/TenantOnly").header(HttpHeaders.AUTHORIZATION, bearerDemo2))
					.andExpect(status().isNotFound())
					.andExpect(jsonPath("$.errorCode").value(ErrorCodes.NOT_FOUND));
		}
		...
		void happyPathReturnsNameAndContent() throws Exception {
			upsert("My Dark", "{\"theme\":\"dark\"}", bearerDemo);
			MvcResult result = mockMvc.perform(get("/api/chart-templates/{name}", "My Dark")
							.header(HttpHeaders.AUTHORIZATION, bearerDemo))
					.andExpect(status().isOk())
					.andExpect(jsonPath("$.name").value("My Dark"))
					.andExpect(jsonPath("$.content").value("{\"theme\":\"dark\"}"))
					.andReturn();
		...
		void getShowsContentAfterUpsert() throws Exception {
			...
			mockMvc.perform(get("/api/chart-templates/RoundTrip").header(HttpHeaders.AUTHORIZATION, bearerDemo))
					.andExpect(status().isOk())
